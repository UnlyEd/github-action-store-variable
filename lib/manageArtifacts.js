...
const manageArtifacts = (variables, delimiter, failIfNotFound) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const artifactToStore = variablesDetail.filter((variable) => variable.operationToProceed === 0).map((variable) => variable.detail);
    core.debug(`Artifact to store: ${JSON.stringify(artifactToStore)}`);
    yield storeArtifact(artifactToStore, failIfNotFound);
    const artifactToRetrieve = variablesDetail.filter((variable) => variable.operationToProceed === 1).map((variable) => variable.detail);
